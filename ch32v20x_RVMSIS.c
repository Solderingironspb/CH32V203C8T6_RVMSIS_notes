...
void RVMSIS_RCC_SystemClock_144MHz(void) {
    ...
    MODIFY_REG(RCC->CFGR0, RCC_CFGR0_MCO, 0b0000 <<RCC_CFGR0_MCO_Pos); //В качестве тактирования для MCO выбрал HSE. Будет 8 MHz.
...
void RVMSIS_TIM1_PWM_CHANNEL1_init(void) {
    ...
    //SET_BIT(AFIO->PCFR1, AFIO_PCFR1_TIM1_REMAP);
...
void RVMSIS_TIM4_init(void) {
    /*Включим тактирование таймера (страница 48)*/
    SET_BIT(RCC->APB1PCENR, RCC_APB1Periph_TIM4); //Запуск тактирования таймера 4
    SET_BIT(RCC->APB2PCENR, RCC_APB2Periph_AFIO); //Запуск тактирования альтернативных функций

    CLEAR_BIT(TIM4->CTLR1, TIM_UDIS); //Генерировать событие Update
    CLEAR_BIT(TIM4->CTLR1, TIM_URS); //Генерировать прерывание
    CLEAR_BIT(TIM4->CTLR1, TIM_OPM); //One pulse mode off(Счетчик не останавливается при обновлении)
    CLEAR_BIT(TIM4->CTLR1, TIM_DIR); //Считаем вниз
    MODIFY_REG(TIM4->CTLR1, TIM_CMS, 0b00 << TIM_CMS_Pos); //Выравнивание по краю
    SET_BIT(TIM4->CTLR1, TIM_ARPE); //Auto-reload preload enable
    MODIFY_REG(TIM4->CTLR1, TIM_CTLR1_CKD, 0b00 << TIM_CTLR1_CKD_Pos); //Предделение выключено

    /*Настройка прерываний (Страница 409)*/
    SET_BIT(TIM4->DMAINTENR, TIM_UIE); //Update interrupt enable
    ...
    SET_BIT(TIM4->CTLR1, TIM_CEN); //Запуск таймера
}
...
void RVMSIS_I2C1_Init(void) {
    ...
    //PB9 SDA (I2C Data I/O) Alternate function open drain
    MODIFY_REG(GPIOB->CFGHR, GPIO_CFGHR_CNF9, 0b11 << GPIO_CFGHR_CNF9_Pos);//Alternate function open drain
    MODIFY_REG(GPIOB->CFGHR, GPIO_CFGHR_MODE9, 0b11 << GPIO_CFGHR_MODE9_Pos); //Maximum output speed 50 MHz
    //PB8 SCL (I2C clock) Alternate function open drain
    MODIFY_REG(GPIOB->CFGHR, GPIO_CFGHR_CNF8, 0b11 << GPIO_CFGHR_CNF8_Pos);//Alternate function open drain
    MODIFY_REG(GPIOB->CFGHR, GPIO_CFGHR_MODE8, 0b11 << GPIO_CFGHR_MODE8_Pos); //Maximum output speed 50 MHz
...
void RVMSIS_SPI1_init(void) {
    ...
    MODIFY_REG(SPI1->CTLR1, SPI_CTLR1_BR, 0b010 << SPI_CTLR1_BR_Pos);//fPCLK/4. 72000000/32 = 2.22 MBits/s
