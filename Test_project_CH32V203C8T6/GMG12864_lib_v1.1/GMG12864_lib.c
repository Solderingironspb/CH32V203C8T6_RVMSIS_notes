...
unsigned char tx_buffer[128] = { 0, }; //Буфер для отправки текста на дисплей
...
/// \param Command - 8 бит данных.
	DC_SET();
	RVMSIS_SPI_Data_Transmit_8BIT(SPI1, &Command, 1, 100);
	DC_RESET();
}
/*----------------------Функция отправки команды на дисплей------------------------*/
...
static void GMG12864_Send_data(uint8_t Dat) {
	RVMSIS_SPI_Data_Transmit_8BIT(SPI1, &Dat, 1, 100);
}
/*----------------------Функция отправки данных на дисплей------------------------*/
...
void GMG12864_Generate_a_Graph(uint8_t *counter, uint8_t *array, uint8_t size_array, bool * array_is_full, int y_min, int y_max, uint8_t x_grid_time, uint8_t time_interval,
bool grid) {
/// Функция, формирующая весь график, который будет отображаться на дисплее
/// \param uint8_t *counter - счетчик значений
...
/// \param bool grid - Сетка граффика. вкл = 1. выкл = 0.
	...
	GMG12864_Decode_UTF8(80, 57, 1, 0, text);//53
	...
	//57 67
	GMG12864_Draw_line(87, 53, 87, 55, 1); //Полоса по х левая
	GMG12864_Draw_line(97, 53, 97, 55, 1); //Полоса по х правая
	GMG12864_Draw_line(82, 54, 102, 54, 1); //Полоса между делениями по х
	GMG12864_Draw_line(84, 52, 84, 56, 1); //Левая стрелка на деление
	GMG12864_Draw_line(85, 53, 85, 55, 1); //Левая стрелка на деление
	GMG12864_Draw_line(99, 53, 99, 55, 1); //Правая стрелка на деление
	GMG12864_Draw_line(100, 52, 100, 56, 1); //Правая стрелка на деление
